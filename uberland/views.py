...
def wait(request):
    requests=RequestRide.objects.get(passenger=Passenger.objects.get(user=request.user))
...
def cancel(request):
    ride_rec=RecieveRide.objects.get(passenger__user=user.request)
    ride_req=RequestRide.objects.get(passenger__user=user.request)
    if canceled:
        ride_rec.delete()
        ride_req.delete()
        return redirect('http://127.0.0.1:8000/request_order/')
    return render('driver_map.html')
